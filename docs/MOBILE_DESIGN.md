...
- **iOS home-screen** meta (`index.html`): `apple-mobile-web-app-capable=yes`, **`apple-mobile-web-app-status-bar-style=black-translucent`** — the app draws edge-to-edge under the status bar, and the shell paints a contrast-safe cap behind the (always-white) status text: navy in the light theme, the dark bg tone in dark. **`black-translucent` is the only value that yields a branded status bar** — on current iOS, `black` and `default` both render the stock status bar (white in light mode; verified on device 2026-07-18), so switching values is never the fix for translucent-mode bugs. Its known WebKit bug — standalone mode leaving layout-viewport slack at the bottom that renders as a **white strip** — is neutralized by the `--canvas` guard: `html` carries a solid theme-matched `background-color` (`tokens.css`), the one paint guaranteed to cover the entire web view. Keep that guard; deleting it brings the strip back. `apple-mobile-web-app-title=Mindscape`, `apple-touch-icon`. **Caveats:** iOS bakes these metas into the installed app at "Add to Home Screen" time — retest only after delete + re-add; iOS has no `beforeinstallprompt` — install is manual "Add to Home Screen"; always test standalone mode separately from the browser tab.
