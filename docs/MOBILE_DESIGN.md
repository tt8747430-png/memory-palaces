...
- **Keyboard (ADR-0010):** `interactive-widget=resizes-visual` — full pages hold still and the keyboard overlays their lower edge; **only Drawers lift** (spec 07-14). Lifting is owned by the primitive: the shared `Drawer` bakes Base UI's `Drawer.VirtualKeyboardProvider` into its anatomy, so every drawer is keyboard-aware by construction. **Hand-made drawers are banned** — one built outside the shared component misses the keyboard handling by definition. **Never hand-roll `visualViewport` keyboard handling at all** (ADR-0010 Amendment 2). Two attempts shipped and both broke on a real iPhone: a `--kb` transform raced iOS's caret-reveal pan and flickered, and the later `useKeyboardPin` + `KeyboardInsetProvider` pair (a) cancelled the primitive's own lift by force-scrolling the document on the same `visualViewport` events, and (b) mistook the home-indicator area for a keyboard in installed mode, leaving a permanent bottom strip. Both are deleted; the primitive owns lifting, full stop. **Form layout rules (these are the supported fix):** top-align form content where the design allows; a centered form must center overflow-safely (`margin-block: auto` spacers — bare `justify-content: center` clips its top when it overflows and becomes unscrollable, which is exactly the login-page trap).
...
- **Inputs:** set the right `inputMode`/`type`/`autocomplete`/`enterKeyHint` so the correct on-screen keyboard and return-key label appear; keep the submit action reachable above the keyboard — inside a Drawer the primitive lifts it; on a full page that is a **layout** obligation (§2), never a measured inset. **Text inputs render at ≥16px** (`--ms-text-title`), baked into the shared `Input`/`Textarea` — this is what prevents iOS focus auto-zoom without blocking pinch (ADR-0010); never shrink an input's font below it.
...
- **iOS home-screen** meta (`index.html`): `apple-mobile-web-app-capable=yes`, **`apple-mobile-web-app-status-bar-style=black`** — **never `black-translucent`** (device-verified 2026-07-18): the translucent value routes standalone WebKit through a legacy path that leaves the layout viewport short at the bottom — the whole app rides up above a permanent dead strip that no CSS can relayout away. **The status-bar color is defense-in-depth, because iOS changed the rules per version** (see the next bullet for the meta layer): recent iOS ignores both status-bar metas in standalone, runs the app edge-to-edge, and **samples the bar tint from the fixed element at the top edge** (WebKit fixed-container edge sampling, `LocalFrameView::fixedContainerEdges`: a hit test at the top-edge midpoint picks the topmost fixed/sticky, ~viewport-wide, opaque element and paints the bar with its `background-color`; shipped WebKit **honors `pointer-events` in that hit test**, so a `pointer-events-none` cap is invisible to the sampler — device-symptom: a toast sliding through the forehead became the sampled container and flipped the bar white). Three layers own the color: (1) the shell's **status-bar cap** (`root-layout.tsx` — `bg-status-bar`, `height: env(safe-area-inset-top)`, collapses to 0 when the inset is 0, **hit-testable on purpose — never give it `pointer-events-none`**) paints the forehead when edge-to-edge — at **`--ms-z-statusbar`, above every in-app layer including toasts** (the sonner `Toaster` is pinned to `--ms-z-toast` for exactly this; a toast sliding through the forehead must never repaint the bar; only the splash, at `--ms-z-splash`, sits higher — its gradient owns the forehead at boot); (2) **`body { background-color: var(--status-bar) }`** (never visible — `#root` covers it) steers the derivation; (3) `--canvas` on `html` (`tokens.css`) guards any viewport slack from rendering white. `apple-mobile-web-app-title=Mindscape`, `apple-touch-icon`. **Caveats:** iOS bakes these metas into the installed app at "Add to Home Screen" time — retest only after delete + re-add; iOS has no `beforeinstallprompt` — install is manual "Add to Home Screen"; always test standalone mode separately from the browser tab.
- **The status bar follows the theme:** `index.html` ships **three** `theme-color` metas — one **bare** (no `media`) plus a `media="(prefers-color-scheme: light|dark)"` pair. Each covers a real engine quirk (device-verified): iOS versions that still honor `theme-color` read the **bare tag only** — with just the media-attributed pair the bar renders stock white — while **dark-mode Chromium ignores a lone bare tag**, so Android needs the attributed pair. The theme provider's sync is **idempotent**: a meta already carrying the right color is left completely untouched (iOS samples theme-color unreliably after DOM churn, and both themes resolve to the same navy, so the common path touches nothing); when a color genuinely changes it **removes and re-appends a fresh node, never mutating `content` in place** (iOS Safari browser mode ignores in-place mutations). The manifest's static `theme_color` covers the splash phase before the page loads. **Verify on a real device** — the iOS simulator misrenders these mechanisms.
