...
- **iOS home-screen** meta (`index.html`): `apple-mobile-web-app-capable=yes`, **`apple-mobile-web-app-status-bar-style=black`** — iOS paints the status bar itself, colored by the **bare** `theme-color` meta (navy), and the web view runs from below it to the true bottom edge, so the app never has to manage the top inset (`env(safe-area-inset-top)` is 0 in standalone). **`black-translucent` is banned** (device-verified 2026-07-18): its edge-to-edge mode triggers a WebKit standalone bug that leaves the layout viewport short at the bottom — the whole app rides up above a permanent dead strip that no CSS can relayout away. The rebuild shipped it with a painted status-bar cap and reverted for exactly this. `--canvas` (`tokens.css`) stays as a last-line guard: `html`'s solid theme-matched `background-color` is the one paint guaranteed to cover the entire web view, so any future viewport slack renders app-colored instead of white. `apple-mobile-web-app-title=Mindscape`, `apple-touch-icon`. **Caveats:** iOS bakes these metas into the installed app at "Add to Home Screen" time — retest only after delete + re-add; iOS has no `beforeinstallprompt` — install is manual "Add to Home Screen"; always test standalone mode separately from the browser tab.
- **The status bar follows the theme:** `index.html` ships **three** `theme-color` metas — one **bare** (no `media`) plus a `media="(prefers-color-scheme: light|dark)"` pair — and the theme provider updates **all three** on every `data-theme` switch — by **removing and re-appending fresh meta nodes, never mutating `content` in place** (iOS Safari browser mode often ignores in-place mutations; recreating the node is the established workaround). Each tag covers a real engine quirk (device-verified): **iOS standalone colors its status bar from the bare tag only** — with just the media-attributed pair the bar renders stock white — while **dark-mode Chromium ignores a lone bare tag**, so Android needs the attributed pair. The manifest's static `theme_color` covers the splash phase before the page loads. **Verify on a real device** — the iOS simulator misrenders these mechanisms.
