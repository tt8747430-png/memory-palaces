...
- **iOS home-screen** meta (`index.html`): `apple-mobile-web-app-capable=yes`, **`apple-mobile-web-app-status-bar-style=black`** — the status bar is opaque black and the web view starts below it. **`black-translucent` is banned:** on-device (installed mode) WebKit mis-sizes the web view under it, leaving a permanent white strip at the bottom edge — the edge-to-edge look plus a painted status-bar cap was tried in the rebuild and reverted for exactly this. `apple-mobile-web-app-title=Mindscape`, `apple-touch-icon`. **Caveats:** iOS has no `beforeinstallprompt` — install is manual "Add to Home Screen"; always test standalone mode separately from the browser tab.
- **The status bar follows the theme:** `index.html` ships **two** `theme-color` metas with `media="(prefers-color-scheme: light|dark)"` — dark-mode Chromium ignores a single un-attributed tag — and the theme provider updates **both** on every `data-theme` switch — by **removing and re-appending fresh meta nodes, never mutating `content` in place** (iOS Safari browser mode often ignores in-place mutations; recreating the node is the established workaround) — so Android's status bar (and desktop PWA chrome) always matches the app regardless of the OS scheme. iOS standalone ignores `theme-color` entirely: there the status bar is the opaque `black` of the meta above. The manifest's static `theme_color` is only the install-time default. **Verify on a real device** — the iOS simulator misrenders both mechanisms.
