...
- **`CONTEXT-MAP.md`** at the repo root if it exists — it points at one `CONTEXT.md` per context. Read each one relevant
  to the topic.
...
> Note: this repo already keeps a canonical glossary at [`docs/UBIQUITOUS_LANGUAGE.md`](../UBIQUITOUS_LANGUAGE.md).
> Treat it as the domain glossary until a root `CONTEXT.md` exists.
...
If the concept you need isn't in the glossary yet, that's a signal — either you're inventing language the project doesn'
t use (reconsider) or there's a real gap (note it for `/domain-modeling`).
