...
| Term           | Definition                                                                                                                                               | Aliases to avoid                          |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| **Drawer**     | A bottom-anchored overlay that rises into the thumb zone; the app's canonical overlay for forms and choices — named after the Base UI primitive it wraps | sheet, bottom sheet, modal, bottom dialog |
| **Snap point** | A preset resting height of a tall Drawer (half ↔ full); every snap point is a designed, verified state                                                   | detent (Apple docs), breakpoint           |
...
- **"palace / room / locus"** — the product is _themed_ as a memory palace (method of loci), but the shipped model is **Deck / Card / Question**. Don't use palace/room/locus in code, UI copy, or discussion; they're legacy design-doc vocabulary.
...
- **"Drawer" vs "sheet"** — our word is **Drawer**, always, matching the Base UI/shadcn primitive the shared component wraps (ADR-0010) so the app vocabulary and the library never diverge. "Sheet" is legacy vocabulary from `main`'s code and older docs (`Sheet`, `openMoveDeckSheet`, …); ported symbols are renamed `*Drawer` mechanically at port time. Never introduce a new "sheet" name.
