...
- **But WebKit re-anchors `position: fixed` to the _visual_ viewport while the keyboard is up**, so the fixed root does _not_ ride off the screen and needs no compensation. **`getBoundingClientRect()` is in that same space — the pan is already baked into every rect.** The probe reading that settles it: a focused field reported `top: 57` while `--vv-top` was `113`; had rects been layout-relative, a field at 57 would have been off the top of the screen instead of plainly visible.
...
So the pan is a given, and **nothing in the app should compensate for it** — the platform already has. The only thing that must know about it is the arithmetic that converts a CSS-pixel measurement into rect space.
...
- **Every keyboard measurement uses `--app-height` as the denominator**, never live `clientHeight`. `--vvh` is deleted. **No chrome consumes `--vv-top`** — not the header, not the status cap. The platform already re-anchors them.
