...
- **Never resize a shell for the keyboard — reveal the field instead** ([ADR 0002](adr/0002-keyboard-covers-the-app.md)). `#root` is `inset: 0`, always the full layout viewport; the keyboard covers its bottom and footers hide behind it. A shell that tracks `visualViewport.offsetTop` moves twice per focus: iOS pans to reveal an occluded field, the shell follows, the field no longer needs revealing, iOS un-pans. That loop _is_ the flicker. `useKeyboardInset` publishes one number, `--kb-inset` (≥120px, so an accessory bar alone doesn't count), and is the only `visualViewport` subscriber.
