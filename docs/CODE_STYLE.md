...
- **Only `transform`ed chrome may consume `--vv-top`, and the reveal must know about it.** `HeaderBar` (→ every screen) and `RootLayout`'s status cap translate by the pan so they stay on screen; a transform moves no field, so Safari's decision can't change and the loop can't restart. But Safari parks the field at the top of the visible area — exactly where the translated header lands — so `useKeyboardReveal` clamps its band to the header's post-transform rect and the footer's top. Ride the pan without clamping and the header appears to swallow the field.
...
- **A page footer is `sticky bottom-0` inside the scroll body — a header, upside down.** Content passes behind it; it comes to rest at its flow position at the end of the scroll. Safe only because the shell is anchored: the scrollport bottom is then the real screen bottom, so the pinned bar sits _behind_ the keyboard, and `--kb-inset` padding is the scroll range that lifts it clear. Pin to a scrollport something can shrink and it lands on the keyboard's edge instead.
