...
- **Never re-lay-out for the keyboard — reveal the field instead** ([ADR 0002](adr/0002-keyboard-covers-the-app.md)). `#root` is `inset: 0`, always the full layout viewport. A shell that tracks `visualViewport.offsetTop` moves twice per focus: iOS pans to reveal an occluded field, the shell follows, the field no longer needs revealing, iOS un-pans. That loop _is_ the flicker. `useKeyboardInset` publishes `--kb-inset` (≥120px, so an accessory bar alone doesn't count) and `--vv-top`, and is the only `visualViewport` subscriber.
- **A pan will happen anyway — ride it with `transform`, never with layout.** iOS reveals a field before `focusin` reaches us and won't un-pan until blur, so a fixed root can sit 113px off the top with its header gone. `HeaderBar` (the one header chrome → every screen) and `RootLayout`'s status cap take `translate-y-[var(--vv-top)]`: a transform moves no field, so iOS's decision can't change and the loop can't restart. **A header must never be off-screen; a bottom-docked `AppNav` deliberately stays put and hides behind the keyboard.**
...
- **A page footer is `sticky bottom-0` _inside_ the scroll body, never a sibling under it.** Same `--kb-inset` padding, two behaviours for free: keyboard down it's pinned to the scrollport bottom like a docked bar; keyboard up the padding is scroll range below it, so scrolling to the end lifts it clear. No branch, no remount, no reflow on keyboard open. The reveal clamps its band to `[data-slot="footer-bar"]`'s top as well as the keyboard's, or a field lands behind the footer at the end of the scroll.
