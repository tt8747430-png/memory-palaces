...
- **`visualViewport.offsetTop` and `getBoundingClientRect()` are different coordinate spaces.** WebKit re-anchors `position: fixed` to the visual viewport while the keyboard is up, so the app does _not_ ride off-screen and rects already carry the pan. **Compensate for it anywhere and you double-count it** — a header translated by `--vv-top` slides down over the content; a band clamped to `--app-height − --kb-inset` ends 113px below the real visible bottom and parks fields under the keyboard. `visibleBottom()` (`= --app-height − --kb-inset − --vv-top`) is the one sanctioned bridge; everything else in `useKeyboardReveal` is rect-vs-rect.
