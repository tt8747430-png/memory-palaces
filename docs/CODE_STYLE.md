...
**All of that is `useSortableBlock()` (`shared/lib`) — don't write it again.** It is headless and owns carry, pile, landing and drop placement; a surface supplies only `sectionOf` (who counts as a peer) and its own markup. `LibrarySelectList` and `ReorderableList` both sit on it. The settings pages keep their own `DndContext` on purpose: horizontal single-item assignment across two buckets is a different interaction, not the same one rendered differently.
