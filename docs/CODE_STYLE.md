...
- **Anchor the shell to the screen, never to the live viewport** ([ADR 0002](adr/0002-keyboard-covers-the-app.md)). `#root` is `height: var(--app-height)` — the layout viewport measured **while no field is focused**, and held there. In a standalone iOS PWA the layout viewport _shrinks_ when the keyboard opens, so anything sized off it (`inset: 0`, `100%`, `100dvh`) reflows on every keyboard — that reflow is the flicker, and it drags every pinned edge up with it.
- **Measure the keyboard against `--app-height`, never `documentElement.clientHeight`.** When the platform shrinks the layout viewport, `clientHeight - visualViewport.height ≈ 0`: the keyboard measures as _absent_, so no padding is added, no field is revealed, nothing is learned for next time — and iOS then pans to reveal the field itself, taking the header off the top. One stale denominator, every symptom. `useKeyboardInset` publishes `--kb-inset` (≥120px, so an accessory bar alone doesn't count) and is the only `visualViewport` subscriber.
...
- **A page footer belongs _in the scroll flow_ — `mt-auto`, never pinned.** `AuthScreen` (the login page) is the reference: it has no pinned chrome at all, so it was the one surface the keyboard never broke. `AppScreen` matches it — the footer is the last flex child of the scroller, sitting at the bottom while the content fits and scrolled to at the end once it doesn't. **Pinned bottom chrome and a keyboard cannot both be right:** anything `sticky`/`fixed` at the bottom pins to the scrollport, and the moment the platform shrinks that scrollport the bar lands on the keyboard's top edge instead of behind it.
