...
import { createPalaceStore, makePalace, type Palace, selectPalaces } from '@/entities/palace'
import { createRoomStore, makeRoom, type Room, roomsForPalace, selectRooms } from '@/entities/room'
import { createLocusStore, lociForRoom, type Locus, makeLocus, selectLoci } from '@/entities/locus'
...
describe('importPalace (round-trips a palace export)', () => {
  it('recreates the palace, its identity, and all rooms with content under fresh ids', async () => {
    ...
    expect(imported[0]?.category).toBe('History')
