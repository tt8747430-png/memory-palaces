...
import { createProgressStore, type Progress } from '@/entities/progress'
import { awardXp, recordQuizResult, recordTrainingDay } from './index'
...
describe('recordTrainingDay', () => {
  ...
  it('is idempotent within the same UTC day', async () => {
    ...
    expect(store.getState().progress?.streakCount).toBe(1)
  })
