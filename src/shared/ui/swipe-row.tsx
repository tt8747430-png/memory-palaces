...
import { impact } from '@/shared/domain'
...
export function SwipeRow({
  ...
  useEffect(() => {
    if (disabled && (open || x.get() !== 0)) {
      // Force-closes the tray when an external `disabled` transition lands
      // mid-swipe — the same reset the gesture handlers already perform on a
      // user-driven close, just triggered by a prop instead of a pointer event.
    ...
    >
      ...
      <motion.span
        ...
        className={cn(
          ...
          accent.ink === 'dark' ? 'text-(--ms-navy-900)' : 'text-white',
        )}
      >
        {action.icon}
      </motion.span>
