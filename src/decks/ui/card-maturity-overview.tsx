...
export function CardMaturityOverview({ total, counts }: CardMaturityOverviewProps) {
  ...
  return (
    <div>
      <p className="mb-2.5 text-[length:var(--ms-text-title)] font-bold tracking-tight text-heading">
      ...
      <ul className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1.5">
        {ORDER.map((k) => (
          <li
            key={k}
            className="inline-flex items-center gap-1.5 text-[length:var(--ms-text-label)] text-muted-foreground"
          >
