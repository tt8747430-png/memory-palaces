...
import { cn, type StickyHeader } from '@/shared/lib'
    ...
    <StickyBar elevation={header.elevation} className="min-h-12">
      ...
      >
        <span className="grid size-12 shrink-0 place-items-center rounded-full border border-[color:var(--border-glass)] bg-card-glass shadow-rest">
          <Avatar name={name} src={avatar} className="size-11 text-[length:var(--ms-text-sub)]" />
        </span>
        <span className="flex min-w-0 flex-1 flex-col gap-1">
          <span className="truncate text-[length:var(--ms-text-sub)] font-bold leading-tight tracking-tight text-heading">
            {t(`home.${greetingKey(new Date().getHours())}`)}
          </span>
          <span className="flex items-center gap-2">
            <span className="shrink-0 text-[length:var(--ms-text-label)] font-semibold text-primary">
              {t('home.level', { level })}
            </span>
            <ProgressBar
              value={fill}
...
function StreakButton({
  ...
  return (
    <button
      ...
      className="inline-flex shrink-0 items-center gap-1 rounded-full bg-card px-2.5 py-1 text-[length:var(--ms-text-label)] font-semibold text-heading shadow-rest transition-transform active:scale-95"
    >
      <Flame
...
function NotificationButton({
  ...
  return (
    <div className="relative shrink-0">
      ...
      {unreadCount > 0 ? (
        <span className="absolute -right-0.5 -top-0.5 grid min-w-[18px] place-items-center rounded-full bg-destructive px-1 text-[length:var(--ms-text-tiny)] font-bold leading-none text-destructive-foreground ring-2 ring-[color:var(--surface)]">
