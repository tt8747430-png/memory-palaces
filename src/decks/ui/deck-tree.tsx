...
import { type DropIntent, dueCountsPerDeck, siblingDecks } from '@/shared/domain'
    ...
    <>
      ...
      <div className="pointer-events-none relative z-10 flex min-w-0 flex-1 items-center gap-3">
        ...
        >
          ...
          {due > 0 ? (
            <span
              className="absolute -right-1.5 -top-1.5 grid h-[18px] min-w-[18px] place-items-center rounded-full bg-primary px-1 text-[length:var(--ms-text-tiny)] font-bold tabular-nums text-primary-foreground shadow-interactive ring-2 ring-card"
              aria-hidden
            >
              {due > 99 ? '99+' : due}
            </span>
        ...
        <span className="min-w-0 flex-1">
          ...
          <span
            className={cn(
              'block truncate text-[length:var(--ms-text-label)]',
