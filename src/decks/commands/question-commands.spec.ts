...
import { questionsForDeck } from '@/decks/model/question'
...
import { QuestionStore } from '@/decks/data/stores'

function startedStore() {
  const store = new QuestionStore(new InMemoryRepository<Question>())
  store.start()
  return store
}
...
const promptsForRoom = (store: ReturnType<typeof startedStore>) =>
  questionsForDeck(store.questions(), 'r1').map((q) => q.prompt)
...
describe('reorderQuestions', () => {
  it('writes each question its index in the supplied order and renumbers the deck', async () => {
    ...
    const byPrompt = (prompt: string) => store.questions().find((q) => q.prompt === prompt)!
