...
import { dayKey } from '@app/shared/domain'
...
describe('completeSession', () => {
  it('awards XP and accumulates the day tally without a streak below goal', async () => {
    ...
    expect(store.progress()?.activeDayCount).toBe(3)
    expect(store.progress()?.trainingDays).toEqual([])
  })
  ...
  it('starts the streak when the goal is reached', async () => {
    ...
    expect(store.progress()?.trainingDays).toContain('2026-01-10')
  })
  ...
  it('records the best quiz accuracy when provided', async () => {
    ...
    expect(store.progress()?.bestQuizAccuracy).toBe(75)
  })
