 ...
 *
 * Deliberately narrow: model types, stores and their repository tokens, and
 * commands. Pages and UI are absent by design, and not an oversight: the
 * composition root (`data.providers.ts`) is eager, so a barrel that reached into
 * pages would drag every lazily-routed deck page into the initial bundle.
 * Components are imported directly by the templates that use them.
 *
