...
import { SESSION_REPOSITORY, SessionStore } from '../data/stores'
import { LocalAuthGateway } from '../data/local-auth-gateway'
...
describe('session commands', () => {
  ...
  it('signUpWithEmail creates an account session and persists the identity', async () => {
    ...
    expect(sessionStore.session()).toMatchObject({ kind: 'account', displayName: 'Ada' })
  ...
  it('continueAsGuest creates a guest session and persists the guest choice', async () => {
    ...
    expect(sessionStore.session()?.kind).toBe('guest')
  ...
  it('restoreSession rehydrates a persisted account', async () => {
    ...
    expect(deps.sessionStore.session()).toMatchObject({ kind: 'account', displayName: 'Ada' })
  })

  it('restoreSession is a no-op when nothing is persisted', async () => {
    const { deps } = setup()
    await restoreSession(deps, AT)
    expect(deps.sessionStore.session()).toBeNull()
  })
