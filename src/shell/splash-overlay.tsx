...
import { WordReveal, Threshold } from '@/shared/ui'
...
const AURA_BG =
  'radial-gradient(circle at center, oklch(var(--ms-tint-sky) / 0.45), transparent 60%)'
...
const SPLASH_BG =
  'linear-gradient(to bottom, var(--ms-navy-900), var(--ms-blue-500) 55%, var(--ms-blue-300))'
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onDone}
        className="absolute right-5 top-[calc(env(safe-area-inset-top)+1rem)] z-10 text-[length:var(--ms-text-label)] font-medium text-white/75"
      >
        {t('auth.splash.skip')}
      </button>
      ...
      <div className="relative z-10 flex flex-col items-center gap-1">
        <WordReveal
          ...
          delay={reduce ? 0 : 0.85}
          className="text-[length:var(--ms-text-headline)] font-semibold tracking-tight text-white"
        />
        <WordReveal
          text={t('auth.splash.tagline')}
          delay={reduce ? 0 : 1.15}
          stagger={0.06}
          className="text-[length:var(--ms-text-sub)] text-white/80"
        />
      </div>
    </motion.div>
  )
