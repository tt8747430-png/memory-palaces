  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-(length:--p-text-label) leading-snug text-muted-foreground">
        ...
        shell rode off the top and is measured as{' '}
        <b>visualViewport top + html rect top − fixed box top</b> — it must be 0 wherever the
        platform kept fixed boxes on screen, or the header is moved twice. <b>focused bottom</b>{' '}
        must stay under <b>reveal band max</b>, and the last row must balance. A fault that only
        shows in motion needs the probe overlay: turn it on in Settings → Developer and record a
        trace on the screen that misbehaves.
      </p>
    </div>
  )
}
