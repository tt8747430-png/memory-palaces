...
import { selectDeckCount, useDeckStore, useDeckStoreApi } from '@/entities/deck'
...
export function useClearData(): ClearData {
  ...
  const stores = useMemo<ResetEverythingStores>(
    () => ({ deckStore, folderStore, cardStore, questionStore, progressStore, notificationStore }),
    [deckStore, folderStore, cardStore, questionStore, progressStore, notificationStore],
  )
  ...
  const decks = useDeckStore(selectDeckCount)
